...
const getNivelLabel = (nivel) => NIVEL_COLOR[nivel]?.label || nivel || '—'
const getNivelBg    = (nivel) => NIVEL_COLOR[nivel]?.bg    || '#fff'
const getNivelColor = (nivel) => NIVEL_COLOR[nivel]?.color || '#333'
...
function Traslados() {
  ...
  // Leer datos del localStorage (cargaData con ETA calculado)
  useEffect(() => {
    try {
      const raw = localStorage.getItem('cargaData')
      if (!raw) return
      const datos = JSON.parse(raw)
      // Solo filas con SERV y H.REC calculado
      const conETA = datos.filter(d => d.col5_serv && d.col11_hrec)
      buildResumen(conETA)
    } catch {}
  ...
  const buildResumen = (datos) => {
    ...
    const rows = Object.entries(servicios).map(([serv, miembros]) => {
      ...
      return {
        serv: Number(serv),
        vuelo, es, hato, pax,
        etaMin, hInicio, hUltima,
        nivel, recorrido, prov, tripulantes, alerta
      }
    })
  ...
  const precisionPct = filas.length > 0
    ? Math.round(((cntN0A + cntN0B) / filas.length) * 100)
    : 0
  ...
  const exportarExcel = () => {
    ...
    const rows = datosFiltrados.map(f => [
      f.serv, f.vuelo, f.es, f.hato, f.pax,
      f.etaMin, f.hInicio, f.hUltima,
      getNivelLabel(f.nivel), f.recorrido, f.prov, f.tripulantes, f.alerta
    ])
    ...
    // Anchos de columna
    ws['!cols'] = [
      {wch:6},{wch:8},{wch:5},{wch:7},{wch:5},{wch:9},{wch:8},{wch:8},
      {wch:14},{wch:45},{wch:14},{wch:50},{wch:14}
    ]
  ...
  const selectStyle = {
    padding: '4px 10px', border: '1px solid #e0e0e0', borderRadius: '6px',
    fontSize: '12px', background: 'white', color: '#333', cursor: 'pointer'
  }
  const thStyle = {
    padding: '8px 10px', color: '#555', textAlign: 'left', fontSize: '11px',
    whiteSpace: 'nowrap', borderBottom: '2px solid #e0e0e0', background: 'white',
    position: 'sticky', top: 0, zIndex: 1, fontWeight: '600'
  }
      ...
      <div style={{ padding: '12px 24px', borderBottom: '1px solid #e0e0e0' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          <div>
            ...
            <p style={{ color: '#888', margin: '2px 0 0', fontSize: '12px' }}>Tiempos estimados de traslado por servicio</p>
          </div>
      ...
      {filas.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid #e0e0e0', flexWrap: 'wrap' }}>
          ...
          <div style={{ padding: '8px 20px', display: 'flex', alignItems: 'center', gap: 16, flex: 1, flexWrap: 'wrap' }}>
            <div style={{ fontSize: 12, color: '#555', fontWeight: 600 }}>Precisión del cálculo:</div>
            {[
            ...
            <div style={{ marginLeft: 8, background: precisionPct >= 70 ? '#e8f5e9' : '#fff3e0', color: precisionPct >= 70 ? '#2e7d32' : '#e65100', padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 700 }}>
