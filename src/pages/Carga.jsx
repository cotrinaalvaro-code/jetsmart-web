...
function Carga() {
  ...
  const handleAgrupar = async () => {
    ...
    try {
      ...
      resultado.sort((a, b) => {
        const hatoA = horaAMin(a._hatoSort), hatoB = horaAMin(b._hatoSort)
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ color: '#888', margin: '2px 0 0', fontSize: '12px' }}>
              Genera el DataCargaM cruzando con la BD de tripulantes
            </p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <label style={{
              padding: '7px 16px', background: '#00b4d8', color: 'white',
            ...
            {datos.length > 0 && (
              <>
                <button onClick={handleAgrupar} disabled={agrupando} style={{
                  padding: '7px 16px', background: agrupando ? '#aaa' : '#00cc88',
                  color: 'white', border: 'none', borderRadius: '6px',
                  fontWeight: '600', fontSize: '13px', cursor: agrupando ? 'not-allowed' : 'pointer'
                }}>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  {showCols && (
                    <div style={{
                      ...
                      border: '1px solid #e0e0e0', borderRadius: '8px', zIndex: 100,
                      boxShadow: '0 4px 16px rgba(0,0,0,0.12)', padding: '12px',
                      width: '200px', maxHeight: '400px', overflowY: 'auto'
                    }}>
                      <div style={{ fontWeight: '700', color: '#333', fontSize: '12px', marginBottom: '10px' }}>
                        Mostrar columnas
                      </div>
                      {COLUMNAS.map(c => (
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #e0e0e0', background: '#fafafa' }}>
          {[
            ...
            <div key={stat.label} style={{
              ...
              borderRight: '1px solid #e0e0e0', background: 'white'
            }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                ...
                {col('grupo') && <th style={thStyle}>GRUPO</th>}
                {col('orden') && <th style={thStyle}>ORDEN</th>}
                {col('corredor') && <th style={thStyle}>CORREDOR</th>}
            ...
            <tbody>
              {datos.map((d, idx) => (
                <tr key={d.uid} style={{
                  ...
                  background: !d.enBD ? '#fff5f5'
                    : d.col25_corredor === 'SIN CORREDOR' ? '#fffde7'
                  ...
                  {col('es') && <td style={tdStyle}>
                    <span style={{
                      padding: '1px 6px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold',
                      background: d.col4_es === 'E' ? '#e8f5e9' : '#e3f2fd',
                      color: d.col4_es === 'E' ? '#2e7d32' : '#1565c0'
                    }}>{d.col4_es}</span>
                  </td>}
                  ...
                  {col('cat') && <td style={tdStyle}>
                    <span style={{ padding: '1px 6px', borderRadius: '4px', background: '#e3f2fd', color: '#1565c0', fontSize: '11px' }}>{d.col13_cat}</span>
                  </td>}
                  ...
                  {col('dist') && <td style={{...tdStyle, color: '#888'}}>{d.col17_dist}</td>}
                  ...
                  {col('grupo') && <td style={{
                    ...
                    color: d.col23_grupo ? (d.col4_es === 'E' ? '#2e7d32' : '#1565c0') : '#aaa',
                    background: d.col23_grupo ? (d.col4_es === 'E' ? '#e8f5e9' : '#e3f2fd') : 'transparent'
                  }}>{d.col23_grupo}</td>}
                  {col('orden') && <td style={{...tdStyle, color: '#333', fontWeight: '700', textAlign: 'center'}}>{d.col24_orden}</td>}
                  {col('corredor') && <td style={tdStyle}>
                    <input value={d.col25_corredor} onChange={e => editarCorredor(d.uid, e.target.value)}
                      style={{
                        background: d.col25_corredor === 'SIN CORREDOR' ? '#fff8e1' : '#f5f5f5',
                        border: `1px solid ${d.col25_corredor === 'SIN CORREDOR' ? '#ffa000' : '#ddd'}`,
                        borderRadius: '4px', color: '#333', padding: '2px 6px', fontSize: '11px', width: '120px'
                      }} />
                  </td>}
                  {col('bd') && <td style={{...tdStyle, textAlign: 'center'}}>
                    {d.enBD ? <span style={{ color: '#2e7d32', fontWeight: 'bold' }}>✓</span>
